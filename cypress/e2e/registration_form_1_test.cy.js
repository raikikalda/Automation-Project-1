...
Assignment 2:
 ...
 2. Replace text ‘Salasona123’ in the first test with your own chosen password (2 places) - passwords should match
...
describe('This is first test suite, Raiki Kalda ', () => {
    it('User can submit data only when valid mandatory values are added', () => {
        //07.07.2024
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('#firstName').type('EsimeneNimi')
        cy.get('#lastName').type('TeineNimi')
        cy.get('input[name="password"]').type('Salasona123')
        cy.get('[name="confirm"]').type('Salasona123')
    ...
    it('User can use only same both first and validation passwords', () => {
        ...
        cy.get('input[name="password"]').type('Salasona123')
        cy.get('[name="confirm"]').type('Salasona123123')
